...
import type { InferredInsertAlbumSchema, InferredInsertArtistSchema, InferredInsertTrackSchema, InferredSelectAlbumSchema, InferredSelectArtistSchema, InferredSelectTrackSchema, ServerLoadDefaultValues } from "$lib/types";

import { logger } from "$lib/logger";
import { albums, artists, tracks } from "$lib/schema";
...
export const load: LayoutServerLoad = async ({ parent }) => {
  ...
  if (currentLibrary) {
    ...
    logger.debug(returnedArtists, "Artists in db");
    ...
    const returnedTracks: Array<InferredSelectTrackSchema> | undefined = await db.query.tracks.findMany({
      where: eq(tracks.library, currentLibrary?.uuid),
    });
    ...
    const _updatedTracks: Array<InferredSelectTrackSchema> = await db.insert(tracks).values(plexAlbumTracks).onConflictDoUpdate({
      target: tracks.uuid,
      set: {
